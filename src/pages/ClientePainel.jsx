...
const Painel = () => {
  ...
  useEffect(() => {
    const fetchFeedbacks = async () => {
      try {
        const token = localStorage.getItem("token"); // Ou outra forma de armazenar o JWT

        const response = await fetch("http://localhost:3000/api/reclamacoes/minhas", {
          headers: {
              ...
              <div style={{ marginTop: '10px' }}>
                ...
                <ul>
                  {fb.anexos.map((anexo, index) => (
                    <li key={index}>
                      <a
                        href={`http://localhost:3000/uploads/${anexo}`}
                        target="_blank"
